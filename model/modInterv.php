...
// poner en la línea 6 GROUP_CONCAT(DISTINCT CONCAT(p.name,'(',DATE_FORMAT(p.startdate, '%Y-%m-%d'),')')ORDER BY p.startdate ASC SEPARATOR '<br>') AS proyectos_fechas,
$sqlDetalle = "
...
GROUP_CONCAT(DISTINCT p.name ORDER BY p.startdate ASC SEPARATOR '<br>') AS proyectos_fechas,
...
JOIN systems m ON j.idsst = m.idsst
JOIN supersystems d ON m.idspr = d.idspr
LEFT JOIN prjact pa ON pa.idprj = p.idprj
$where
GROUP BY
    d.name,
    m.name,
